...
if(isset($_POST['submit'])){
       ...
       $number = mysqli_real_escape_string($conn,$_POST["nomor"]); 
	   $text = strtolower($_POST["alamat"]); 
	   $address = base64_encode($text);
        ...
        $imagetype	= mysqli_real_escape_string($conn, $_FILES["gambar"]["type"]);
   		if(substr($imagetype,0,5)== "image"){
		   			...
		   			$sql = mysqli_query($conn,"UPDATE data SET email='$email', alamatrumah='$address', nomor='$number', img='$img' WHERE username='$ya'") or die(mysqli_error($conn));
if (($sql) === TRUE) {
...
var loadFile = function(event) {
	...
	image.src = URL.createObjectURL(event.target.files[0]);
			...
			<tr>
			   <td>
			   <input type="number" class="form-control" id="nomor" name="nomor" autofocus placeholder="nomor" value="<?php echo $no;?>"><input type="text" class="form-control" id="alamat" name="alamat" autofocus placeholder="Alamat" value="<?php echo $alamatrumah;?>">
			   </td>
			</tr>
			<tr>
			   <td>
